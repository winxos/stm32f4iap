...
#define H 160
#define WxH 20480
u16 front_color=0x0f00;
u16 background=0;//0x1042;
  ...
  for(u8 r=0;r<16;r++)
	{
		for(u8 c=0;c<8;c++)
		{
			...
			{
				lcd_point(x+c,y+r,front_color);
			}
			else
			{
				lcd_point(x+c,y+r,background);
			}
		}
	}
}
...
void lcd_vline(u8 x1,u8 y1, u8 x2,u8 y2)
{
	...
	for(u8 j=ymin;j<ymax;j++)
	{
		lcd_point(x1,j,front_color);
	}
}	
